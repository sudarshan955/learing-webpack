...
<script src="http://127.0.0.1/menu/node_modules/jquery/dist/jquery.js"></script>
    <script src="http://127.0.0.1/menu/node_modules/underscore/underscore.js"></script>
  ...
  },
  {
    ...
    "ContainsProducts": true
			...
			var html = '';
			var renderMenu = function(listItem, filterIndex, sortByIdentifier) {
				html += '<ul>';
				//console.log('<ul>');
				//document.write('<ul>');				
				_.sortBy( _.where( listItem, { ParentCategoryId : filterIndex } ), sortByIdentifier ).forEach( function(value, index, subListItem) {
					html += '<li>' + value.Name + '</li>';
					//console.log('<li>' + value.Id + '</li>');
					//document.write('<li>' + value.Name + '</li>');
					( _.where( listItem, { ParentCategoryId : value.Id } ).length ) ? renderMenu( listItem, value.Id, sortByIdentifier ) : null;
				} );
				html += '</ul>';
				//console.log('</ul>');
				//document.write('</ul>');
			...
			var tHtml = renderMenu( _.sortBy( menuList, 'ParentCategoryId' ), 0, 'DisplayOrder' );
